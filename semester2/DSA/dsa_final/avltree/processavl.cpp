...
void clear_exist_ID(set<string> &listset2,struct avl_table *avl){
	set<string>::iterator ti;
	int count = 0;
	for(ti = listset2.begin(); ti != listset2.end(); ti++){
		INDEX<THistory> tmp(*ti);
		if(avl_find(avl, &tmp) != NULL)
			//cout <<"clear "<< *ti << endl;
			listset2.erase(ti);
		else
			count ++;
...
void listing10(bool exist, string &ID,struct avl_table *avl){
	int i;
	struct avl_traverser *si = (struct avl_traverser *)malloc(sizeof(struct avl_traverser));
	INDEX<THistory> *get;
	set<TL> listset;
	set<TL>::iterator ti;
	if(exist){
		avl_t_init(si, avl);
		avl_t_first(si, avl);
		while(avl_t_cur(si) != NULL){
			get = (INDEX<THistory> *)avl_t_cur(si);
...
void FIND(string user, string test, struct avl_table *avl){
	int mode = -1;
//	bool found = false;
...
void processLogin(struct avl_table *avl,customer** user_now){
	...
	INDEX<THistory> *tmp = (INDEX<THistory> *)avl_find(avl, &new_index); 
	if(tmp == NULL)
...
void processCreate(struct avl_table *avl){
	...
	void **position = avl_probe(avl, new_index);
	if(*position == new_index){
		cout <<"success"<< endl;
	}else{
		cout <<"ID "<< ID <<" exists, ";
		listing10(false, ID, avl);
		delete new_index;
	}
}
...
void processMerge(struct avl_table *avl){
	...
	INDEX<THistory> *tmp1 = (INDEX<THistory> *)avl_find(avl, &new_index1);
	INDEX<THistory> *tmp2 = (INDEX<THistory> *)avl_find(avl, &new_index2);
	if(tmp1 == NULL)
...
void processFind(customer* user_now, struct avl_table *avl){
	...
	string wild_card_ID;
	cin >> wild_card_ID;
	//FIND(user_now->ID, wild_card_ID, avl);
	cout << endl;
}
