...
void listing10(bool exist, string &ID,struct rb_table *rb){
	int i;
	struct rb_traverser *si = (struct rb_traverser *)malloc(sizeof(struct rb_traverser));
	INDEX<THistory> *get;
	set<TL> listset;
	set<TL>::iterator ti;
	if(exist){
		rb_t_init(si, rb);
		rb_t_first(si, rb);
		while(rb_t_cur(si) != NULL){
			get = (INDEX<THistory> *)rb_t_cur(si);
...
void FIND(string user, string test, struct rb_table *rb){
	int mode = -1;
//	bool found = false;
...
void processLogin(struct rb_table *rb,customer** user_now){
	...
	INDEX<THistory> *tmp = (INDEX<THistory> *)rb_find(rb, &new_index); 
	if(tmp == NULL)
...
void processCreate(struct rb_table *rb){
	...
	void **position = rb_probe(rb, new_index);
	if(*position == new_index){
		cout <<"success"<< endl;
	}else{
		cout <<"ID "<< ID <<" exists, ";
		listing10(false, ID, rb);
		delete new_index;
	}
}
...
void processMerge(struct rb_table *rb){
	...
	INDEX<THistory> *tmp1 = (INDEX<THistory> *)rb_find(rb, &new_index1);
	INDEX<THistory> *tmp2 = (INDEX<THistory> *)rb_find(rb, &new_index2);
	if(tmp1 == NULL)
...
void processFind(customer* user_now, struct rb_table *rb){
	...
	string wild_card_ID;
	cin >> wild_card_ID;
	//FIND(user_now->ID, wild_card_ID, rb);
	cout << endl;
}
